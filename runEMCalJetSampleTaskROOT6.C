...
//#include "tracks/AliAnalysisTaskEmcalPatchesRef.h"
...
AliAnalysisManager* runEMCalJetSampleTaskROOT6(
    const char   *cDataType      = "AOD",                                   // set the analysis type, AOD or ESD
    const char   *cRunPeriod     = "LHC12e",                                // set the run period
    const char   *cLocalFiles    = "filesLHC12eAOD.txt",   // set the local list file filesLHC15MC
    const UInt_t  iNumEvents     = 50000,                                    // number of events to be analyzed
    ...
    const UInt_t  iNumFiles      = 8,                                     // number of files analyzed locally
...
)
{
  ...
  if (iDataType == kEsd) {
    AliPhysicsSelectionTask *pPhysSelTask = reinterpret_cast<AliPhysicsSelectionTask*>(gInterpreter->ExecuteMacro("$ALICE_PHYSICS/OADB/macros/AddTaskPhysicsSelection.C"));
  }

  // Centrality task
  if (iDataType == kEsd && iBeamType != AliAnalysisTaskEmcal::kpp) {
    //AliCentralitySelectionTask *pCentralityTask = AddTaskCentrality(kTRUE);
     AliCentralitySelectionTask *pCentralityTask = reinterpret_cast<AliCentralitySelectionTask*>(gInterpreter->ExecuteMacro("$ALICE_PHYSICS/OADB/macros/AddTaskCentrality.C"));
    pCentralityTask->SelectCollisionCandidates(AliVEvent::kAny);
  }

  // CDBconnect task
  if (bDoFullJets || iDataType == kEsd) {
    AliTaskCDBconnect *taskCDB = reinterpret_cast<AliTaskCDBconnect*>(gInterpreter->ExecuteMacro("$ALICE_PHYSICS/PWGPP/PilotTrain/AddTaskCDBconnect.C"));
  ...
  if (bDoEmcalCorrections) {
    ...
    AliEmcalCorrectionTask * correctionTask = reinterpret_cast<AliEmcalCorrectionTask*>(gInterpreter->ExecuteMacro("$ALICE_PHYSICS/PWG/EMCAL/macros/AddTaskEmcalCorrectionTask.C"));
  ...
  if (iBeamType != AliAnalysisTaskEmcal::kpp) {
    ...
    pKtChJetTask->SelectCollisionCandidates(kPhysSel);
      ...
      AliAnalysisTaskRho* pRhoTask  = reinterpret_cast<AliAnalysisTaskRho*>(gInterpreter->ExecuteMacro("$ALICE_PHYSICS/PWGJE/EMCALJetTasks/macros/AddTaskRhoNew.C(\"usedefault\", \"usedefault\", \"\", 0.4)"));
      pRhoTask->SetOutRhoName(sRhoChName);
    ...
    //PID Maker
    AliAnalysisTaskPIDResponse *taskPID = reinterpret_cast<AliAnalysisTaskPIDResponse*>(gInterpreter->ExecuteMacro("$ALICE_ROOT/ANALYSIS/macros/AddTaskPIDResponse.C"));
    
    //PID qa
    AliAnalysisTaskPIDqa *PIDqa = reinterpret_cast<AliAnalysisTaskPIDqa*>(gInterpreter->ExecuteMacro("$ALICE_ROOT/ANALYSIS/macros/AddTaskPIDqa.C"));
    
    //TriggerMaker
    AliEmcalTriggerMaker *emcalTriggers = reinterpret_cast<AliEmcalTriggerMaker*>(gInterpreter->ExecuteMacro("$ALICE_PHYSICS/PWG/EMCAL/macros/AddTaskEmcalTriggerMaker.C"));
    
    //Trigger Patch Ref
    //AliAnalysisTaskEmcalPatchesRef *emcalTriggerPatchRef = reinterpret_cast<AliAnalysisTaskEmcalPatchesRef*>(gInterpreter->ExecuteMacro("$ALICE_PHYSICS/PWGJE/EMCALJetTasks/macros/AddTaskEmcalPatchesRef.C"));
    
    
    cout<<"BACKGROUND JETS"<<endl;
  ...
  if (bDoFullJets) {
    ...
    cout<<"Sample Task"<<endl;
  // Sample task
  AliAnalysisTaskEmcalJetSample *sampleTask = reinterpret_cast<AliAnalysisTaskEmcalJetSample*>(gInterpreter->ExecuteMacro("$ALICE_PHYSICS/PWGJE/EMCALJetTasks/macros/AddTaskEmcalJetSample.C(\"usedefault\", \"usedefault\", \"usedefault\", \"new\")"));
  ...
  sampleTask->SelectCollisionCandidates(kPhysSel);
    ...
    //8 TeV task
    AliAnalysisTaskEmcalJetSpectra8TeVTriggerQA *jetSpectraQA = reinterpret_cast<AliAnalysisTaskEmcalJetSpectra8TeVTriggerQA*>(gInterpreter->ExecuteMacro("$ALICE_PHYSICS/PWGJE/EMCALJetTasks/macros/AddTaskEmcalJetSpectra8TeVTriggerQA.C(\"usedefault\", \"usedefault\", \"usedefault\", \"new\")"));
    
    jetSpectraQA->GetClusterContainer(0)->SetClusECut(0.);
    jetSpectraQA->GetClusterContainer(0)->SetClusPtCut(0.);
    jetSpectraQA->GetClusterContainer(0)->SetClusNonLinCorrEnergyCut(0.);
    jetSpectraQA->GetClusterContainer(0)->SetClusHadCorrEnergyCut(0.30);
    jetSpectraQA->GetClusterContainer(0)->SetDefaultClusterEnergy(AliVCluster::kHadCorr);
    jetSpectraQA->GetParticleContainer(0)->SetParticlePtCut(0.15);
    jetSpectraQA->SetHistoBins(600, 0, 300);
    jetSpectraQA->SelectCollisionCandidates(kPhysSel);
   
  // AliJetResponseMaker *ResponseMaker = reinterpret_cast<AliJetResponseMaker*>(gInterpreter->ExecuteMacro("$ALICE_PHYSICS/PWGJE/EMCALJetTasks/macros/AddTaskJetResponseMaker.C"));
  ...
  if (bDoFullJets) {
    ...
    AliJetContainer* jetCont04 = sampleTask->AddJetContainer(AliJetContainer::kFullJet, AliJetContainer::antikt_algorithm, AliJetContainer::pt_scheme, 0.4, AliEmcalJet::kEMCALfid, "Jet");
      ...
      AliJetContainer* jetCont02Spectra = jetSpectraQA->AddJetContainer(AliJetContainer::kFullJet, AliJetContainer::antikt_algorithm, AliJetContainer::pt_scheme, 0.2, AliEmcalJet::kEMCALfid, "Jet");
      AliJetContainer* jetCont04Spectra = jetSpectraQA->AddJetContainer(AliJetContainer::kFullJet, AliJetContainer::antikt_algorithm, AliJetContainer::pt_scheme, 0.4, AliEmcalJet::kEMCALfid, "Jet");
    ...
    if (iBeamType != AliAnalysisTaskEmcal::kpp) {
      ...
      jetCont04->SetPercAreaCut(0.6);
        ...
        jetCont02Spectra->SetRhoName(sRhoFuName);
        jetCont02Spectra->SetPercAreaCut(0.6);
        jetCont04Spectra->SetRhoName(sRhoFuName);
        jetCont04Spectra->SetPercAreaCut(0.6);
    }
  }
  ...
  pMgr->SetUseProgressBar(kTRUE, 1000);
  ...
  if (iStartAnalysis == 1) { // start local analysis
    ...
    if (iDataType == kAod) {
      #if !defined (__CINT__) || defined (__CLING__)
        // FIXME correct pointer type
        pChain = reinterpret_cast<TChain*>(gInterpreter->ExecuteMacro(Form("$ALICE_PHYSICS/PWG/EMCAL/macros/CreateAODChain.C(\"%s\", %i, 0, kFALSE, \"\", 0)", sLocalFiles.Data(), iNumFiles)));
        cout<<"Path1: "<<endl;
      #else
       ...
       #if !defined (__CINT__) || defined (__CLING__)
//        gInterpreter->ExecuteMacro("$ALICE_PHYSICS/PWG/EMCAL/macros/CreateESDChain.C(sLocalFiles.Data(), iNumFiles, 0, kFALSE)");
      #else
    ...
    Printf("Starting Analysis...");
      cout<<"pChain: "<<pChain<<"  NumEvents:" <<iNumEvents<<endl;
    ...
    StartGridAnalysis(pMgr, cTaskName, cGridMode);
      cout<<"YOURE STARTING A GRID ANALYSIS"<<endl;
  }
...
void LoadMacros()
{
...
#if !defined (__CINT__) || defined (__CLING__)
    /*
    gInterpreter->LoadMacro("$ALICE_PHYSICS/OADB/macros/AddTaskCentrality.C");
    ...
    gInterpreter->LoadMacro("$ALICE_PHYSICS/PWGJE/EMCALJetTasks/macros/AddTaskEmcalJetSample.C");
    gInterpreter->LoadMacro("$ALICE_PHYSICS/PWGJE/EMCALJetTasks/macros/AddTaskRhoNew.C");
    gInterpreter->LoadMacro("$ALICE_PHYSICS/OADB/macros/AddTaskPhysicsSelection.C");
    gInterpreter->LoadMacro("$ALICE_PHYSICS/PWG/EMCAL/macros/AddTaskEmcalTriggerMaker.C");
    ...
    gInterpreter->LoadMacro("$ALICE_PHYSICS/PWG/EMCAL/macros/AddTaskEmcalTriggerMakerNew.C");
    gInterpreter->LoadMacro("$ALICE_PHYSICS/PWG/EMCAL/macros/AddTaskEmcalTriggerMaker.C");
    gInterpreter->LoadMacro("$ALICE_PHYSICS/PWGJE/EMCALJetTasks/macros/AddTaskEmcalJetSpectra8TeVTriggerQA.C");
    gInterpreter->LoadMacro("$ALICE_PHYSICS/PWGJE/EMCALJetTasks/macros/AddTaskEmcalPatchesRef.C");
     */
    ...
    gROOT->LoadMacro("$ALICE_PHYSICS/PWG/EMCAL/macros/AddTaskEmcalTriggerMaker.C");
    gROOT->LoadMacro("$ALICE_PHYSICS/PWGJE/EMCALJetTasks/macros/AddTaskEmcalJetSpectra8TeVTriggerQA.C");
    gROOT->LoadMacro("$ALICE_PHYSICS/PWGJE/EMCALJetTasks/macros/AddTaskEmcalPatchesRef.C");
